    ...
    <section className="w-full py-12 md:py-16 px-4 md:px-12 lg:px-[114px] bg-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {teams.map((team, index) => (
          <div
            key={index}
            className="h-56 perspective group"
          >
            <div
              className="relative w-full h-full transition-transform duration-500 transform-gpu group-hover:[transform:rotateY(180deg)]"
              style={{
            ...
            >
              ...
              >
                <h3 className="font-swiss font-medium text-black text-lg md:text-xl lg:text-[22px] tracking-[-0.5px] leading-tight text-center whitespace-pre-line">
                  {team.title}
                </h3>
              </div>

              <div
                className="absolute w-full h-full flex flex-col items-center justify-center border border-black p-4 md:p-6 bg-white"
                style={{
                  backfaceVisibility: "hidden",
                  transform: "rotateY(180deg)",
                }}
              >
                <p className="font-swiss font-normal text-black text-xs md:text-sm lg:text-[14px] tracking-[-0.4px] leading-relaxed text-center">
                  {team.description}
                </p>
              </div>
            </div>
      ...
      <p className="text-center text-gray-400 text-sm mt-8 font-swiss italic">
        Survolez les cartes pour découvrir ce que chaque équipe a apporté au projet
      </p>
    </section>
  );
};
